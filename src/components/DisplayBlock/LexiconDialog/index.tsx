...
} from 'stores/LexiconEntryDialogStore';
...
export default () => {
  const [marginToCenter, setMarginToCenter] = createSignal({ left: 0, top: 0 });

  let dialogTitle: HTMLDivElement | ((el: HTMLDivElement) => void) | undefined,
    dialogContainer: HTMLDivElement | ((el: HTMLDivElement) => void) | undefined,
    dialogMarkup: HTMLDivElement | ((el: HTMLDivElement) => void) | undefined,
    lexiconEntryDialogCloseButton:
      | HTMLButtonElement
  ...
  onMount(() => {
    // createRenderEffect(() => {
    //   // For some reason .focus() is not recognized in HTMLButtonElement that
    //   // inherits the methods of HTMLElement.
    //   if (lexiconEntryDialog.isOpen) {
    //     console.log('portal opened');
    //     (lexiconEntryDialogCloseButton as HTMLElement).focus();
    //   }
    // });
    //
    createEffect(() => {
      if (lexiconEntryDialog.isOpen) {
        ...
        if (dialogContainer !== null) {
          const left =
            ...
            dialogContainer.clientWidth / 2;
          const top =
            // @ts-ignore // the check was made
            dialogContainer.clientHeight / 2 - document.body.clientHeight / 2;
          setMarginToCenter({
            left,
            top,
          });
    ...
    createEffect(() => {
      if (lexiconEntryDialog) {
        createDraggable(dialogContainer as HTMLElement, dialogTitle as HTMLElement);
  ...
  const portalContainer = document.getElementById('portal') as Node | undefined;

  // Reset the lexiconEntryDialog global state.
  const closeLexiconEntryDialog = () => {
    setLexiconEntryDialog((current) => {
      current.isOpen = false;
      current.lexiconIndex = '';
  ...
  return (
    <Portal mount={portalContainer}>
      <LexiconEntryDialogContainer ref={dialogContainer}>
        <LexiconEntryDialogTitle ref={dialogTitle}>
          {lexiconEntryDialog.lexiconIndex}
          <LexiconEntryDialogCloseButton
            ref={lexiconEntryDialogCloseButton}
            onClick={closeLexiconEntryDialog}
          >
            Close
          </LexiconEntryDialogCloseButton>
        </LexiconEntryDialogTitle>
        <LexiconEntryDialogMarkupContainer innerHTML={lexiconEntryDialog.lexiconEntry} />
        <LexiconEntryDialogFooter />
      </LexiconEntryDialogContainer>
    </Portal>
  );
