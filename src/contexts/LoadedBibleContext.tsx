...
const LoadedBibleProvider: FC<ReactNode> = ({ children }) => {
	...
	const reducer = (
		...
		action: LoadedBibleType & LoadedBibleInfoType & LoadedBibleOtherType & WordIndexProps
	) => {
		switch (action.type) {
			case 'setBibleObject':
			...
			case 'setTranslatedWordFromBibleObject':
				const copyOfBibleObject = {
					...
					[state.bibleInfo.bibleBookName]: [
						...
						][state.bibleInfo.bibleVerseIndex][action.wordIndex].splice(
							0,
							1,
			...
			case 'setBibleBookNameFromBibleInfo':
				return {
					...
					bibleInfo: {
						...state.bibleInfo,
						bibleBookName: action.bibleBookName,
						bibleChapterIndex: '0',
						bibleVerseIndex: '0',
					},
				};
			case 'setBibleChapterIndexFromBibleInfo':
				return {
					...
					bibleInfo: {
						...state.bibleInfo,
						bibleChapterIndex: action.bibleChapterIndex,
						bibleVerseIndex: '0',
					},
				};
			case 'setBibleVerseIndexFromBibleInfo':
				return {
					...state,
					bibleInfo: { ...state.bibleInfo, bibleVerseIndex: action.bibleVerseIndex },
				};
			case 'setBibleWordIndexFromBibleInfo':
				return {
					...state,
					bibleInfo: { ...state.bibleInfo, bibleWordIndex: action.bibleWordIndex },
				};
			default:
	...
	return (
		<LoadedBibleContext.Provider value={value}>{children}</LoadedBibleContext.Provider>
	);
};
